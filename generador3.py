...
nombreparalels = int(sys.argv[4])
densitatpredecessors = int(sys.argv[5])

f = open('provagenerador3_' + str(nombrellibres) + '_' + str(nombredesitjats) + '_' + str(nombrellegits) + '_' + str(nombreparalels) + '_' + str(densitatpredecessors) + '.pddl', 'w')

G = nx.gnp_random_graph(nombrellibres, (densitatpredecessors/10), directed=True)

DAG = nx.DiGraph([(u, v, {'weight': random.randint(1, 1)}) for (u, v) in G.edges() if u < v])
nx.is_directed_acyclic_graph(DAG)
nx.nodes(DAG)

f.write('(define (problem ext3_problema)\n  (:domain ext3_domini)\n  (:objects')
f.write('\n')
...
for i in range(nombrellibres):
    u = topological_sorted[i-1]
    pags = random.randint(50, 799)
    f.write('\n  (= (pagines llibre_' + str(u) + ') ' +  str(pags) + ')')
f.write('\n')

p = 0
for i in range(len(topological_sorted)):
    if (p < nombreparalels and i % 2 == 0 and (i + 1) < len(topological_sorted)):
